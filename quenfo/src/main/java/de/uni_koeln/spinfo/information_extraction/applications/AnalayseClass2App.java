...
public class AnalayseClass2App {
	...
	//ComptenceData sollte in Klasse zwei am besten ohne InnerSentenceSplitting erstellt werden, da vollständige Sätze benötigt werden
	
	//extrahiert alle Verben (= alle PosTags, die mit "V" beginnen) 
	//inkl. deren Subjekte (= erster WordNode im Subtree mit Role "SB")  und
	//inkl. aller Objekte (= alle WordNodes im Subtree mit Role "OA", "OA2", "DA", "OC" oder "OG"   und  alle WordNodes mit Role "OP" oder "MO" inkl. aller NNs im Subtree)
	...
	private static List<String> getSubjVerbObj(DependencyTree tree) {
		...
		List<WordNode> verbNodes = new ArrayList<WordNode>();
		...
		for (WordNode verbNode : verbNodes) {
	...
	private static WordNode findSubject(WordNode verbNode) {
		if(verbNode.getChilds()!= null){
			for (WordNode child : verbNode.getChilds()) {
				...
				findObject(child, new ArrayList<WordNode>());
			}
		}
		return null;
	}
	...
	private static List<WordNode> findObject(WordNode verbNode, List<WordNode> obj) {
		if(verbNode.getChilds()!= null){
			for (WordNode child : verbNode.getChilds()) {
				if(child.getRole().equals("OA")||child.getRole().equals("OA2")||child.getRole().equals("DA")||child.getRole().equals("OC")||child.getRole().equals("OG") ){
					obj.add(child);
				}
	...
	private static void getVerbNodes(WordNode node, List<WordNode> verbNodes) {
